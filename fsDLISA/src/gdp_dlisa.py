...
class d_spacetime_analytics:
    ...
    def d_spacetime_analytics_main(self, config):
        ...
        #Example data:
	#	code,UNCODE,yr2000num,yr2011num
	#	784,784,34395,45653
	#	4,4,90,543
	#	12,12,1794,5244
	...
	UNCODE = [line[1] for line in lines[1:]]
        ...
        data = np.array([map(float,line[2:]) for line in lines[1:]])
        ...
        t1 = sorted([float(line[2]) for line in lines[1:]])
        t2 = sorted([float(line[3]) for line in lines[1:]])
        ...
        output.write("ids,UNCODE,yr2000,yr2011,theta,dx,dy,zdx,zdy,abs_dist,percentile_cat,lisa_cat\n")
        for x in range(len(theta)):
            ...
            output.write(str(ids[x])+","+str(UNCODE[x])+","+str(t1[x])+","+str(t2[x])+","+str(theta[x])+","+str(dx[x])+","+str(dy[x])+","+str(zdx[x])+","+str(zdy[x])+","+str(abs_dist[x])+","+str(percentile_cat)+","+str(lisa_cat)+"\n")
        output.close()
...
if __name__ == "__main__":
    ...
    config['input_csv'] = "./gdp_input.csv"              #Input CSV
    config['input_weights'] = "./fsGDP_DLISA2knn8.gwt"   #Input GWT
    config['output_csv'] = "./gdp_output_knn8.csv"       #Output file name to be written to
